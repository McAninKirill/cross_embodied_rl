...
@dataclasses.dataclass
class HGCMultiGoalDataset(HGCDataset):
    ...
    def sample(self, batch_size, idxs=None, relabeling=True, augmentation=True):
        ...
        if not np.isclose(self.config['actor_p_randomgoal'], 0.0):
            raise ValueError('HGCMultiGoalDataset currently requires actor_p_randomgoal=0.')
        if not np.isclose(self.config['actor_p_curgoal'], 0.0):
            raise ValueError('HGCMultiGoalDataset currently requires actor_p_curgoal=0.')

        final_state_idxs = self.terminal_locs[np.searchsorted(self.terminal_locs, idxs)]
        if self.config['actor_geom_sample']:
            offsets = np.random.geometric(p=1 - self.config['discount'], size=batch_size)
            high_goal_idxs = np.minimum(idxs + offsets, final_state_idxs)
        else:
            distances = np.random.rand(batch_size)
            first_future_idxs = np.minimum(idxs + 1, final_state_idxs)
            high_goal_idxs = np.round(
                first_future_idxs * distances + final_state_idxs * (1 - distances)
            ).astype(int)

        num_subgoals = self.config['num_subgoals']
        fractions = np.arange(1, num_subgoals + 1, dtype=np.float32) / (num_subgoals + 1)
        goal_distances = high_goal_idxs - idxs

        waypoint_offsets = np.ceil(goal_distances[:, None] * fractions[None, :]).astype(np.int64)
        waypoint_idxs = np.minimum(idxs[:, None] + waypoint_offsets, high_goal_idxs[:, None])

        predecessor_idxs = np.concatenate([idxs[:, None], waypoint_idxs[:, :-1]], axis=1)
        transition_masks = (waypoint_idxs > predecessor_idxs).astype(np.float32)
        target_masks = np.broadcast_to(
            (goal_distances > 0)[:, None],
            waypoint_idxs.shape,
        ).astype(np.float32)

        batch['high_actor_goals'] = self.get_observations(high_goal_idxs)
        batch['high_actor_targets'] = self.get_observations(waypoint_idxs)
        batch['high_actor_target_masks'] = target_masks
        batch['high_actor_transition_masks'] = transition_masks
        return batch
